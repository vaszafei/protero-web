              ...
              <div class="bg-surface-light/30 rounded-xl p-4">
                ...
                <div class="flex items-center gap-3 mt-3 pt-3 border-t border-edge/20">
                  <div class="flex-1 text-center">
                    <span class="text-sm font-semibold text-zinc-200 tabular-nums">{{ player?.fgm ?? 0 }}/{{ player?.fga ?? 0 }}</span>
                    <span class="text-[10px] text-zinc-500 ml-1">FG</span>
                  </div>
                  <div class="flex-1 text-center">
                    <span class="text-sm font-semibold tabular-nums" :class="(player?.fg3m ?? 0) > 0 ? 'text-orange-400' : 'text-zinc-400'">{{ player?.fg3m ?? 0 }}/{{ player?.fg3a ?? 0 }}</span>
                    <span class="text-[10px] text-zinc-500 ml-1">3PT</span>
                  </div>
                  <div class="flex-1 text-center">
                    <span class="text-sm font-semibold text-zinc-200 tabular-nums">{{ player?.ftm ?? 0 }}/{{ player?.fta ?? 0 }}</span>
                    <span class="text-[10px] text-zinc-500 ml-1">FT</span>
                  </div>
                  <div class="flex-1 text-center">
                    <span class="text-sm font-semibold tabular-nums" :class="(player?.pm ?? 0) > 0 ? 'text-emerald-400' : (player?.pm ?? 0) < 0 ? 'text-red-400' : 'text-zinc-400'">{{ (player?.pm ?? 0) > 0 ? '+' : '' }}{{ player?.pm ?? 0 }}</span>
...
const playerName = computed(() => {
  ...
  return formatName(props.player.name)
})
...
async function loadSeason() {
  ...
  const pid = getPlayerId(props.player)
  try {
